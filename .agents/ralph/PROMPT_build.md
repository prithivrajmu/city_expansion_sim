...
10. If No-commit is false, commit changes using the `$commit` skill if available; otherwise use direct non-interactive git commands.
    - Stage everything: `git add -A`
    - Commit with a conventional message that matches the story scope.
...
A frontend story is NOT complete until browser verification passes. If the browser skill is unavailable, fall back to the repo’s normal dev and build verification path and record that explicitly in progress notes.
